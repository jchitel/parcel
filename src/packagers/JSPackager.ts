...
export default class JSPackager extends Packager {
    first!: boolean;
    dedupe!: Map<none, none>;
    bundleLoaders!: Set<none>;
    ...
    async addAsset(asset: Asset<unknown, unknown>): Promise<void> {
        ...
        let deps: { [name: string]: Array<string | [string, string]> } = {};
        for (let [dep, mod] of asset.depAssets) {
            // For dynamic dependencies, list the child bundles to load along with the module id
            if (dep.dynamic) {
                let bundles: Array<string | [string, string]> = [this.getBundleSpecifier(mod.parentBundle)];
    ...
    dedupeKey(asset: Asset<unknown, unknown>): string {
    ...
    async writeModule(id: number | string, code: string, deps: { [name: string]: Array<string | [string, string]> } = {}, map?: none): Promise<void> {
    ...
    async writeBundleLoaders(): Promise<boolean | undefined> {
