import { SourceMapConsumer, SourceMapGenerator, RawSourceMap, Mapping, Position, FindPosition, MappingItem } from 'source-map';
...
export default class SourceMap {
    ...
    constructor(mappings?: Mapping[] | string, sources?: { [source: string]: string }) {
        this.mappings = this.purifyMappings(mappings);
        this.sources = sources || {};
        this.lineCount = null;
    }

    purifyMappings(mappings?: Mapping[] | string): Mapping[] {
    ...
    async getConsumer(map: string | SourceMapConsumer | RawSourceMap): Promise<SourceMapConsumer> {
        if (map instanceof SourceMapConsumer) {
            return map;
        }
        map = typeof map === 'string' ? JSON.parse(map) as RawSourceMap : map;
        return await new SourceMapConsumer(map);
    }

    async addMap(map: SourceMap | RawSourceMap, lineOffset: number = 0, columnOffset: number = 0): Promise<SourceMap> {
        if (!(map instanceof SourceMap) && map.version) {
            ...
            if ((consumer as any).destroy) {
                // Only needs to happen in source-map 0.7
                (consumer as any).destroy();
            }
        } else {
            if (!('eachMapping' in map)) {
                map = new SourceMap(map.mappings, map.sources as any); // fuck this
            }
            ...
            Object.keys(map.sources).forEach(sourceName => {
                if (!this.sources[sourceName]) {
                    this.sources[sourceName] = (map.sources as any)[sourceName];
                }
            });
        }
    ...
    addMapping(mapping: Mapping, lineOffset: number = 0, columnOffset: number = 0) {
        this.mappings.push({
    ...
    addConsumerMapping(mapping: MappingItem, lineOffset: number = 0, columnOffset: number = 0) {
        if (
            !mapping.source ||
    ...
    eachMapping(callback: (mapping: Mapping) => void) {
    ...
    async extendSourceMap(original: SourceMap | SourceMapConsumer, extension: SourceMap | RawSourceMap) {
        ...
        if ((original as any).destroy) {
            // Only needs to happen in source-map 0.7
            (original as any).destroy();
        }

        return this;
    }

    findClosestGenerated(line: number, column: number) {
        if (line < 1) {
    ...
    findClosest(line: number, column: number, key: 'generated' | 'original'): number | undefined {
        if (line < 1) {
            throw new Error('Line numbers must be >= 1');
        }
    ...
    generatedPositionFor(originalPosition: Position) {
    ...
    sourceContentFor(fileName: string, _returnNullOnMissing?: boolean): string {
