...
type Aliases = { [filename: string]: (string | false) };
...
export interface PackageJson {
    ...
    module?: string | { [name: string]: string };
}
...
interface ResolverOptions {
    extensions?: string[] | { [ext: string]: any };
    rootDir: string;
...
export default class Resolver {
    ...
    rootPackage: PackageJson | null | undefined;

    constructor(options: ResolverOptions = {} as ResolverOptions) {
    ...
    async resolve(input: string, parent: string): Promise<ResolverResult> {
        let filename = input;
    ...
    async resolveModule(filename: string, parent: string): Promise<ResolvedModule> {
        let dir = parent ? path.dirname(parent) : process.cwd();
    ...
    getCacheKey(filename: string, parent: string): string {
        return (parent ? path.dirname(parent) : '') + ':' + filename;
    }

    resolveFilename(filename: string, dir: string): string {
        switch (filename[0]) {
            case '/':
    ...
    async loadRelative(filename: string, extensions: string[]): Promise<ResolvedFile | undefined> {
    ...
    async findNodeModulePath(filename: string, dir: string) {
        if (filename in builtins) {
            ...
            return { filePath: builtins[filename as keyof typeof builtins] };
        }
    ...
    async loadNodeModules(module: ResolvedModule, extensions: string[]) {
        try {
    ...
    async loadDirectory(dir: string, extensions: string[], pkg?: PackageJson): Promise<ResolvedFile | undefined> {
        try {
    ...
    async readPackage(dir: string): Promise<PackageJson> {
        let file = path.join(dir, 'package.json');
        if (this.packageCache.has(file)) {
            return this.packageCache.get(file)!;
        }

        let json = (await fs.readFile(file, 'utf8')) as string;
        let pkg = JSON.parse(json) as PackageJson;
    ...
    getBrowserField(pkg: PackageJson): PackageJson['browser'] | null {
        let target = this.options.target || 'browser';
        return target === 'browser' ? pkg.browser : null;
    }

    getPackageEntries(pkg: PackageJson): string[] {
    ...
    async loadAsFile(file: string, extensions: string[], pkg: PackageJson | undefined): Promise<ResolvedFile | undefined> {
    ...
    expandFile(file: string, extensions: string[], pkg: PackageJson | undefined, expandAliases: boolean = true): string[] {
    ...
    resolveAliases(filename: string, pkg: PackageJson | undefined): string {
    ...
    resolvePackageAliases(filename: string, pkg: PackageJson | null | undefined): string {
        if (!pkg) {
            return filename;
        }
    ...
    getAlias(filename: string, dir: string, aliases: string | Aliases | null | undefined): string | null {
        if (!filename || !aliases || typeof aliases !== 'object') {
            return null;
        }

        let alias: string | false;
    ...
    lookupAlias(aliases: Aliases, filename: string, dir: string): string | false {
        // First, try looking up the exact filename
        let alias: string | false = aliases[filename];
        if (alias == null) {
            // Otherwise, try replacing glob keys
            for (let key in aliases) {
                if (isGlob(key)) {
                    let re = micromatch.makeRe(key, { capture: true } as any); // types are wrong?
                    if (re.test(filename)) {
                        alias = filename.replace(re, aliases[key] as string); // why?
                        break;
                    }
                }
            }
    ...
    async findPackage(dir: string): Promise<PackageJson | undefined> {
    ...
    async loadAlias(filename: string, dir: string): Promise<string> {
