...
import bundleReport from './utils/bundleReport';
...
export default class Bundler extends EventEmitter {
    ...
    watchedAssets: Map<string, Asset>;
    farm: WorkerFarm | null;
    watcher: Watcher | null;
    hmr: HMRServer | null;
    bundleHashes: none | null;
    error: none | null;
    buildQueue: PromiseQueue;
    rebuildTimeout: none | null;
    entryAssets?: Set<Asset>;
    mainBundle?: Bundle;

    constructor(entryFiles: string | string[] | undefined, options: BundlerInputOptions = {}) {
        super();
    ...
    normalizeEntries(entryFiles: string | string[] | undefined): string[] {
        ...
        return (entryFiles as string[])
            .reduce((p, m) => p.concat(glob.sync(m)), [])
            .map(f => Path.resolve(f));
    }

    normalizeOptions(options: BundlerInputOptions): BundlerOptions {
        const isProduction =
            options.production || process.env.NODE_ENV === 'production';
        const publicURL = (options as any).publicUrl || options.publicURL || '/';
        const watch =
    ...
    addPackager(type: string, packager: Packager): void {
        if (this.farm) {
    ...
    addBundleLoader(type: string, paths: string | BundleLoaderConfig): void {
